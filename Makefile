...
ifeq ($(SERVICE),m3ctl)
	...
	make build-ui-ctl-statik-packages
endif
...
ifeq ($(shell ls ./src/ctl/ui/build 2>/dev/null),)
	...
	make cmd-using-node-version node_cmd="cd $(m3_package_path)/src/ctl/ui && yarn install && npm run build"
else
...
.PHONY: build-ui-ctl-statik-packages
build-ui-ctl-statik-packages: build-ui-ctl install-tools
	mkdir -p ./src/ctl/generated/ui
	$(retool_bin_path)/statik -src ./src/ctl/ui/build -dest ./src/ctl/generated/ui -p statik
